...
.cat-row{cursor:pointer}.cat-row[data-selected=true]{background:var(--cx-hover)}.cat-lane{font-weight:600}.cat-source{color:var(--cx-muted)}
...
function Detail({ event }: { readonly event: TraceEvent | undefined }) {
  ...
  const fields: readonly [string, unknown][] = [
    ...
    ['Entity', definition?.identity.agentId],
    ['Entity revision', definition?.identity.revision],
    ['Entity digest', definition?.digest],
  ...
  return (
    <>
      ...
      {event.payload === undefined ? null : <pre className="cat-payload">{JSON.stringify(event.payload, null, 2)}</pre>}
    </>
  )
}
...
export function createTraceReactPage(createStore: (sessionId: string) => TraceShowcaseStore) {
  return function AgentTracePage(props: CordisXReactPageProps) {
    ...
    const events = orderTraceEvents(filterTraceEvents(snapshot.events, filters), order).slice(
      -snapshot.range.renderedLimit,
    )
    const selected = snapshot.events.find(event => event.id === selectedId)
    const update = <Key extends keyof TraceFilters>(key: Key, value: TraceFilters[Key]) =>
      setFilters(current => ({ ...current, [key]: value }))
    return (
      <div className="cat-root" data-agent-trace-showcase="true" data-read-only="true">
        <style>{STYLES}</style>
        <div className="cat-status" data-state={snapshot.status.mode} title={snapshot.status.diagnostics.join('\n')}>
          <strong>{snapshot.status.mode}</strong>
          {' · '}
          {snapshot.status.completeness}
          {' · '}read-only{' · '}
          {snapshot.range.loaded}/{snapshot.range.totalAvailable ?? snapshot.range.loaded} events
        </div>
        <Stack className="cat-toolbar" direction="row" gap="small" align="center" wrap>
          ...
          <Select
            ...
            value={filters.lane}
            options={options(LANES, 'All lanes')}
            onChange={value => update('lane', value as TraceFilters['lane'])}
          />
          <Select
          ...
          <Select
            ...
            value={filters.phase}
            options={options(phases, 'All phases')}
            onChange={value => update('phase', value as TraceFilters['phase'])}
          />
        </Stack>
        {snapshot.status.mode === 'unavailable'
          ? <EmptyState title="Session events unavailable" description={snapshot.status.diagnostics.join(' · ')} />
          : events.length === 0
          ? (
            <EmptyState
              title="No projected events"
              description="No events in the loaded read-only session window match these filters."
            />
          )
          : (
            <div className="cat-table-scroll">
              <table className="cat-table">
                ...
                <tbody>
                  {groupTraceEvents(events).map(group => (
                    <Fragment key={group.key}>
                      ...
                      {group.events.map(event => (
                        <tr
                          ...
                          onKeyDown={input => {
                            if (input.key === 'Enter' || input.key === ' ') setSelectedId(event.id)
                          }}
                        >
                          <td>{event.seq}</td>
                          <td className="cat-lane">{event.lane}</td>
                          <td>{event.semanticType}</td>
                          <td>{event.phase ?? '—'}</td>
                          <td>
                            {event.summary} <span className="cat-source">{event.source.label}</span>
                          </td>
                        </tr>
                      ))}
                    </Fragment>
        ...
        <aside className="cat-detail" aria-label="Event detail">
