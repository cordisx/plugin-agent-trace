...
import { projectSessionEvent, SessionTraceStore } from '../src/session-store.js'
...
function page(afterSeq: number, snapshotSeq: number, events: readonly SessionEvent[]): SessionEventPage {
  ...
  return {
    $schema:
      'https://raw.githubusercontent.com/cordisx/cordisx-protocol/main/schemas/session-event-page.v1.schema.json',
...
describe('SessionTraceStore', () => {
    ...
    const initial = [
      ...
      event(2, 'assistant/message', {
        ...
        message: {
          ...
          source: { kind: 'model', provider: 'openai', model: 'codex' },
        },
      }),
    ]
  ...
  it.each(
    [
      ...
      'permission-revoked',
      'connection-replaced',
    ] satisfies readonly SessionSubscriptionCloseCode[],
  )('fails closed when the Host terminates an active subscription with %s', async code => {
    const harness = sessionHarness()
    const store = new SessionTraceStore(harness.sessions, 'session-a', 50)
    await store.settled()
  ...
  it('keeps missing, unauthorized, and unavailable capabilities as honest empty states', async () => {
    ...
    expect(missing.getSnapshot()).toMatchObject({
      events: [],
      status: { mode: 'unavailable', diagnostics: ['session-missing-or-unauthorized'] },
    })
    ...
    expect(unavailable.getSnapshot()).toMatchObject({
      events: [],
      status: { mode: 'unavailable', diagnostics: ['session-subscribe-unavailable:host-unavailable'] },
    })
  })
})
